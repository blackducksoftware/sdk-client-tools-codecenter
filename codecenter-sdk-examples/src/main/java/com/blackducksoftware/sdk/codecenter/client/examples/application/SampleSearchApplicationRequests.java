...
public class SampleSearchApplicationRequests extends SampleSearch<RequestSummary> {

    public static void main(String[] args) {
        new SampleSearchApplicationRequests(args).run();
    }

    @Argument(index = 3, required = true, metaVar = "applicationName", usage = "the name of the application to search for requests for")
    protected String applicationName;

    @Argument(index = 4, required = true, metaVar = "applicationVersion", usage = "the application version to search for requests for")
    protected String applicationVersion;

    @Argument(index = 5, required = false, metaVar = "searchString", usage = "part of the component name to search for; omit/make blank to retrieve all")
    protected String searchString;

    @Argument(index = 6, required = false, multiValued = true, handler = ApprovalStatusEnumOptionHandler.class, metaVar = "approvalStatus", usage = "TODO")
    ...
    public SampleSearchApplicationRequests(String[] args) {
        super(args);
    }
    ...
    protected Collection<RequestSummary> search(CodeCenterServerProxy proxy) throws Exception {
        ...
        // Search for requests
        return proxy.getApplicationApi().searchApplicationRequests(appNameVersion, searchString != null ? searchString : "", pageFilter);
    }
    ...
    protected String getSearchExpression() {
        ...
        return sb.toString() + " requests for application \"" + applicationName + "\" version \"" + applicationVersion + "\""
                + (searchString != null ? " matching \"" + searchString + "\"" : "");
    }
