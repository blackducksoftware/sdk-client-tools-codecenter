...
public class SampleUpdateRoleGrantPermissions extends BDCodeCenterSample {

    public static void main(String[] args) {
        new SampleUpdateRoleGrantPermissions(args).run();
    }
    ...
    public SampleUpdateRoleGrantPermissions(String[] args) {
        super(args);
    }
    ...
    public void run(CodeCenterServerProxy proxy) throws Exception {
        ...
        permissionsDelta.removeAll(originalRole.getPermissions());

        RoleUpdate role = new RoleUpdate();
        role.setName(roleNameToken);
        role.getPermissions().addAll(originalRole.getPermissions());
        role.getPermissions().addAll(permissionsDelta);

        // Update the role
        proxy.getRoleApi().updateRole(role);

        // Print success information
        System.out.println((permissionsDelta.isEmpty() ? "Granted " : "Successfully granted ")
                + (permissionsDelta.size() != 1 ? permissionsDelta.size() + " new permissions"
